...
export async function GET(req: Request): Promise<Response> {
  ...
  try {
    ...
    if (pendingError) throw new Error(`No se pudieron leer pagos pendientes: ${pendingError.message}`)
    ...
    if (expiredError) throw new Error(`No se pudieron expirar pagos antiguos: ${expiredError.message}`)
    ...
    const message = err instanceof Error ? err.message : String(err)
    console.error('[cron/flow-reconcile] Error fatal:', message)
    await recordHeartbeat('flow-reconcile', 'error', message)
