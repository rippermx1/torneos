...
- ✅ **Watchdog de crons**: tabla `cron_heartbeats` (migración `20260706000000`); los 3 crons registran latido (fail-open); endpoint `/api/cron/watchdog` (Supabase Cron horario, respaldo diario en Vercel) alerta si un job está vencido (process-tournaments >15min, resto >30min) o en error; workflows GH con paso `if: failure()` → `/api/cron/report-failure` (alerta inmediata). `ALERT_EMAIL` tiene prioridad y, si falta, se usa el email del owner/admin.
